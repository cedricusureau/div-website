...
export default {
  ...
  if (!props.filteredContactData || props.filteredContactData.length === 0 || 
    ...
    console.log("Données insuffisantes pour findValidRows:", {
      filteredDataLength: props.filteredContactData?.length,
      selectedPositionsLength: props.selectedPositions?.length
    });
    return results;
  }
  
  console.log("selectedPositions:", props.selectedPositions); // Ajouter ce log
  
  props.filteredContactData.forEach(row => {
    // Convertir explicitement en chaîne pour cohérence
    const rowPositionStr = String(row.ResidueID);
    // Vérifiez si cette position est incluse dans les positions sélectionnées
    const positionMatches = props.selectedPositions.includes(rowPositionStr);
  ...
  console.log(`Lignes valides trouvées: ${results.length} sur ${props.filteredContactData.length}`);
  return results;
};
    ...
    const drawSankey = (data) => {
      ...
      // Calculer l'espacement des nœuds en fonction du nombre total de nœuds
      const nodeCount = data.nodes.length;
      const adaptiveNodePadding = Math.max(15, Math.min(30, 45 - nodeCount * 0.8));
      
      // Configuration du diagramme Sankey avec espacement adaptatif
      const sankey = d3Sankey.sankey()
        .nodeWidth(15)
        .nodePadding(adaptiveNodePadding) // Espacement adaptatif
        .nodeId(d => d.index)
      ...
      try {
        ...
        // Calculer la hauteur minimale des nœuds en fonction du nombre de nœuds
        const minNodeHeight = Math.max(15, 30 - nodeCount * 0.5);
        
        // Rectangles des nœuds avec hauteur minimale
        nodeGroups.append('rect')
          .attr('height', d => Math.max(minNodeHeight, d.y1 - d.y0))
        ...
        nodeGroups.append('text')
          .attr('x', d => {
            ...
            return d.name.startsWith('MHC_') ? -8 : 18;
          })
